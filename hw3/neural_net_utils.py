...
def pretty_print_nn_param_list(nn_param_list_of_dict, prefix=''):
    ''' Create pretty display of the parameters at each layer
    '''
    with np.printoptions(formatter={'float': '{: 0.3f}'.format}):
        for ll, layer_dict in enumerate(nn_param_list_of_dict):
            print("Layer %d" % ll)
            print("  %sw | shape %9s | %s" % (prefix, layer_dict['w'].shape, layer_dict['w'].flatten()))
            print("  %sb | shape %9s | %s" % (prefix, layer_dict['b'].shape, layer_dict['b'].flatten()))
